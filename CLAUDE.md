# Claude Code Project Instructions — openssl-sys-kotlin

## Project Overview

This is **openssl-sys-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`openssl-sys`](https://crates.io/crates/openssl-sys).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/openssl-sys/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/openssl-sys>

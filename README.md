...
Kotlin Multiplatform line-by-line clean-room port of the Rust crate [`openssl-sys`](https://crates.io/crates/openssl-sys).

> **Status: scaffold — porting has not started.** This repo currently contains build infrastructure only. The upstream Rust source for the `openssl-sys` crate will be cloned into `tmp/` (gitignored) when porting begins.
...
Original work copyrighted by the upstream `openssl-sys` authors. Kotlin port copyright (c) 2026 Sydney Renee and The Solace Project.
